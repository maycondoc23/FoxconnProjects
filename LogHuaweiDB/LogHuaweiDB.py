...
def coletar_e_enviar(arquivo, pasta):
    try:
        ...
        for item in data:
            ...
            #  pega serialnumber, pcba_sn, datetime, starttime, endtime, stationid e gera um md5 desses dados, caso eles ja tenham sido inseriods na lista_md5-adicionados, conmtinu
            md5_data = f"{SerialNumber}{pcba_sn}{datetime}{starttime}{endtime}{stationid}"
            #  gera md5 de md5_data
            md5_hash = hashlib.md5(md5_data.encode()).hexdigest()
            print(f"MD5 Hash: {md5_hash}")
            if md5_hash in lista_md5_adicionados:
                print(f"Dados já processados: {md5_hash}")
                print(f"Dados já processados: {md5_hash}")
                print(f"Dados já processados: {md5_hash}")
                print(f"Dados já processados: {md5_hash}")
                print(f"Dados já processados: {md5_hash}")
                ...
                lista_md5_adicionados.append(md5_hash)
...
import random
def loop_pasta(pasta):
    folder_path = Path(pasta)  # Convert to Path object
    print("varrendo pasta:", folder_path)
    try:
        try:
...
while True:
    for item in folder_path.iterdir():
        if item.is_dir() and item.name.startswith("ST-"):
            loop_pasta(item)
    lista_md5_adicionados.clear()
